...
const CreditDashboard = () => {
  ...
  const handleTransferCredits = async () => {
    try {
      ...
      if (data.success) {
        setAlert({ type: 'success', message: 'Credits transferred successfully!' });
        setOpenTransferDialog(false);
        setTransferForm({ recipientEmail: '', amount: '', message: '' });
        fetchCreditInfo(); // Refresh credit info
      } else {
        setAlert({ type: 'error', message: data.error || 'Failed to transfer credits' });
      }
    } catch {
  ...
  const getTransactionIcon = (type: string) => {
    switch (type) {
      case 'earned': return <Add sx={{ color: '#4CAF50' }} />;
      case 'spent': return <Remove sx={{ color: '#f44336' }} />;
      case 'bonus': return <Star sx={{ color: '#FF9800' }} />;
      case 'transfer': return <SwapHoriz sx={{ color: '#2196F3' }} />;
  ...
  if (loading) {
    return (
      <Box sx={{ p: 3, textAlign: 'center' }}>
        <CircularProgress />
        <Typography variant="h6" sx={{ mt: 2 }}>Loading credit information...</Typography>
      </Box>
    );
  }
      ...
      <Card sx={{ mb: 4 }}>
        <CardContent>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
            ...
            />
            <Typography variant="h5" sx={{ color: getLevelColor(creditInfo.level), fontWeight: 'bold' }}>
          ...
          <Typography variant="body2" color="text.secondary">
            Total Sessions: {creditInfo.totalEarned} taught • Achievements: {creditInfo.achievements.length}
          </Typography>
        </CardContent>
      </Card>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', py: 2 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    ...
                    <Box>
                      ...
                      <Typography variant="body2" color="text.secondary">
                        {new Date(transaction.date).toLocaleDateString()} • {transaction.creditType} credits
                      </Typography>
                    </Box>
                  </Box>
                  <Box sx={{ textAlign: 'right' }}>
                    <Typography 
                      ...
                      fontWeight="bold"
                      sx={{ 
                        color: transaction.type === 'earned' || transaction.type === 'bonus' ? '#4CAF50' : '#f44336' 
                      }}
                    >
