...
const skillCategories = {
  ...
  'Marketing': [
    ...
    'SEO', 'SEM', 'Email Marketing', 'Brand Strategy', 'Copywriting',
    'Analytics', 'PPC Advertising', 'Influencer Marketing'
  ],
  'Data Science': [
    'Machine Learning', 'Data Analysis', 'Statistics', 'SQL', 'Excel',
    'Tableau', 'Power BI', 'R', 'Pandas', 'NumPy', 'TensorFlow', 'PyTorch',
    'Data Visualization', 'Big Data', 'AI'
  ],
  'Business': [
    'Project Management', 'Leadership', 'Strategic Planning', 'Finance',
    'Accounting', 'Sales', 'Customer Service', 'Operations', 'Consulting',
    'Entrepreneurship', 'Business Analysis', 'Agile', 'Scrum'
  ],
  'Creative': [
    'Video Editing', 'Photography', 'Content Creation', 'Writing',
    'Storytelling', 'Music Production', 'Podcasting', 'Creative Writing',
    'Art', 'Illustration', 'Film Making'
  ],
  'Languages': [
  ...
  'Other': [
    'Communication', 'Public Speaking', 'Teaching', 'Mentoring',
    'Time Management', 'Problem Solving', 'Critical Thinking',
    'Networking', 'Research', 'Presentation Skills'
  ]
};
...
export default function ProfilePage() {
  ...
  const [skillDialogType, setSkillDialogType] = useState<'learning' | 'teaching'>('learning');
      ...
      >
        ...
        <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <Button
            ...
            sx={{
              background: isEditing ? 'transparent' : 'linear-gradient(135deg, #007BFF 0%, #6A0DAD 100%)',
              '&:hover': {
                background: isEditing ? 'rgba(0, 123, 255, 0.1)' : 'linear-gradient(135deg, #0056CC 0%, #4A0080 100%)',
              },
            }}
          >
        ...
        <Paper sx={{ p: 4, mb: 4, borderRadius: 3 }}>
          ...
          <Box sx={{ mb: 4 }}>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, minHeight: 60, p: 2, border: '1px dashed #ccc', borderRadius: 2 }}>
              <AnimatePresence>
                {formData.skillsLearning.map((skill) => (
                  ...
                  >
                    <Chip
                      ...
                      onDelete={isEditing ? () => handleRemoveSkill(skill, 'learning') : undefined}
                    />
                  </motion.div>
                ))}
              </AnimatePresence>
              ...
              {formData.skillsLearning.length === 0 && (
                <Typography variant="body2" color="text.secondary" sx={{ alignSelf: 'center' }}>
                  No learning skills added yet. {isEditing && 'Click "Add Skill" to get started!'}
                </Typography>
              )}
            </Box>
          </Box>
          ...
          <Box>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, minHeight: 60, p: 2, border: '1px dashed #ccc', borderRadius: 2 }}>
                ...
                {formData.skillsTeaching.map((skill) => (
                  ...
                  >
                    <Chip
                      ...
                      variant="outlined"
                      deleteIcon={isEditing ? <X size={16} /> : undefined}
                      onDelete={isEditing ? () => handleRemoveSkill(skill, 'teaching') : undefined}
                    />
                  </motion.div>
                ))}
              </AnimatePresence>
              ...
              {formData.skillsTeaching.length === 0 && (
                <Typography variant="body2" color="text.secondary" sx={{ alignSelf: 'center' }}>
                  No teaching skills added yet. {isEditing && 'Click "Add Skill" to get started!'}
                </Typography>
              )}
            </Box>
          </Box>
                  ...
                  <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                    {skillCategories[selectedCategory as keyof typeof skillCategories].map((skill) => (
                      <Chip
                        ...
                        sx={{ cursor: 'pointer' }}
                        color={
                          (skillDialogType === 'learning' ? formData.skillsLearning : formData.skillsTeaching)
                            .includes(skill) ? 'primary' : 'default'
                        }
                      />
                    ))}
                  </Box>
